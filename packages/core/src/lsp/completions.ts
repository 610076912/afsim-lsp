...
import type { TokenSlice } from "../parser/token-slicer.js";
import type { ScriptCstEntry } from "../parser/orchestrator.js";
import {
  getWsfKeywordsForContext,
  WSF_BOOLEAN_KEYWORDS,
  SCRIPT_ENTRY_KEYWORDS,
} from "../data/wsf-keywords.js";
import type { WsfBlockContext } from "../data/wsf-keywords.js";
import { getAllUnitStrings } from "../data/unit-definitions.js";
import { getTypesForComponent } from "../data/wsf-structs.js";
import type { ComponentKeyword } from "../data/wsf-structs.js";
import {
  BUILTIN_FUNCTIONS,
  SYSTEM_VARIABLES,
...
} from "../data/script-builtins.js";

// ---------------------------------------------------------------------------
// Completion item types (mirrors LSP CompletionItemKind subset)
// ---------------------------------------------------------------------------
...
// ---------------------------------------------------------------------------
// Trigger context — what triggered the completion
// ---------------------------------------------------------------------------
...
interface ScriptContextInfo {
  slice: TokenSlice;
  entry: ScriptCstEntry;
  injectedContextType: string | null;
}

function findScriptContext(
  offset: number,
  slices: readonly TokenSlice[],
  scriptEntries: Map<IToken, ScriptCstEntry>,
): ScriptContextInfo | null {
...
const BLOCK_KEYWORD_TO_CONTEXT: ReadonlyMap<string, WsfBlockContext> = new Map([
...
function computeWsfCompletions(
  ...
  if (prevToken) {
    ...
    if (prevPrevToken && COMPONENT_TYPE_KEYWORDS.has(prevPrevToken.image.toLowerCase())) {
      const kw = prevPrevToken.image.toLowerCase() as ComponentKeyword;
...
function determineWsfContext(offset: number, allTokens: IToken[]): WsfBlockContext {
  ...
  const blockStack: WsfBlockContext[] = [];
...
function computeScriptCompletions(ctx: ScriptContextInfo): CompletionItem[] {
  const items: CompletionItem[] = [];
...
function computeDotCompletions(
  offset: number,
  ctx: ScriptContextInfo,
  allTokens: IToken[],
  text: string,
): CompletionItem[] {
...
function getMethodCompletions(className: string, staticOnly?: boolean): CompletionItem[] {
...
function findTokenBefore(offset: number, allTokens: IToken[]): IToken | null {
...
function mapInjectedContextToType(injectedContextType: string | null): string | undefined {
  if (!injectedContextType) return undefined;
