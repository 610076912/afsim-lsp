...
// ---------------------------------------------------------------------------
function makeMatchResult(matchedText, startOffset, text) {
 ...
 */
function looksLikeFunctionSignature(text, afterScript) {
 ...
 */
function matchScriptFuncEntry(text, startOffset) {
 ...
 */
function matchScriptStmtEntry(text, startOffset) {
    const remaining = text.substring(startOffset);
    // Use WSF suffix assertion to prevent matching "script-xxx"
    const m = /^script(?![A-Za-z0-9_\-\/])/.exec(remaining);
    if (!m)
